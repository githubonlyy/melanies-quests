...
// page has to live in storage or closing the board silently drops her back to
// the blank sheet. Same key family as the other saved preferences.
const PAGE_KEY = 'melanies-quests-draw-page'
 ...
 * toss for a seven-year-old. Where the names collide the file name is shown
 ...
 * of the shelf — is left exactly as it was. Only what she SEES changes; the page
...
export default function Draw({ onClose }) {
  ...
  // derived from that below, so the page she just tapped can never be shown
  ...
  // `templateId` is already initialized above, so the page she reopens on comes
  // back with its lines dark even if she left it faded yesterday
  ...
  // faint — at full strength the photo would bury her painting. Art that needed
  // no keying (vector, or a picture that arrived transparent already) follows
  // her own קווים/שקוף choice like a keyed page does.
  const overlayFailed = !!overlay && (overlay.failed === true || overlay.status === 'failed')
  // exactly what she sees on screen; composeDrawing gets this same number so
  ...
  // out loud and put her back on the blank page instead of a white void.
  ...
  // per session, and the eighth page she opened was enough to spend it, so the
  // second time a page was evicted a perfectly good picture got the buzz and she
  ...
  useEffect(() => {
    ...
    loadPageOverlay(template)
      ...
      .catch((err) => {
        if (!alive) return
        // Keying failed — fall back to the untouched picture, faint, so she can
        ...
        // instead of leaving her staring at white.
  ...
  // the picked page outlives the board; closing it must not drop her to blank
  useEffect(() => {
  ...
  const save = async () => {
    ...
    try {
      ...
      const { dataUrl, w, h } = await composeDrawing({
        ...
        // bake the page at the opacity she is looking at, not at full black
      ...
      <div className="flex items-center gap-2 px-2 py-2 bg-(--t-side) border-b-4 border-(--t-side-deep) shrink-0">
        ...
        {isImagePage &&
          (pageMode === PAGE_MODES.lines ? (
            ...
            <LabeledBtn onClick={togglePageMode} label="דף שקוף" hint="דף שקוף — הקווים יחווירו כדי שתוכלי לצייר מעליהם">
