...
export default function Shop() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center gap-3 bg-(--t-panel) p-4 rounded-2xl border-4 border-(--t-panel-border) backdrop-blur-sm">
        <div>
          <h2 className="text-2xl md:text-3xl font-black text-white drop-shadow-md">פרסים אמיתיים 🎁</h2>
          <p className="text-(--t-text-soft) font-bold text-sm md:text-base mt-1">
            מטבעות קונים פרסים אמיתיים — אמא ואבא מביאים!
          </p>
        </div>
        <div className="hidden md:flex items-center gap-2 bg-black/20 px-4 py-2 rounded-xl shrink-0">
        ...
        {shopItems.map((item) => {
          ...
          return (
            <div key={item.id} className="relative bg-white rounded-3xl border-8 border-slate-200 overflow-hidden shadow-xl flex flex-col group">
              ...
              <div className={`${item.bgColor} p-2 text-center border-b-4 border-black/10 flex flex-col items-center leading-none gap-0.5`}>
              ...
              <div className="p-6 flex flex-col items-center text-center flex-1 relative bg-gradient-to-b from-slate-50 to-slate-200">
                ...
                <span className="text-xs font-bold text-slate-400 mb-1">{item.type}</span>
                <h3 className="text-2xl font-black text-slate-800 leading-tight mb-3">{item.title}</h3>
              ...
              <div className="p-4 bg-slate-100 border-t-4 border-slate-200 mt-auto">
                <button
                  ...
                  className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl font-black text-xl tabular-nums transition-all
                    ${canAfford
