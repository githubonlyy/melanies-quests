...
tablet, and the app buzzes, says **"הדף הזה לא נטען"** and drops her back to the
blank sheet.

## Naming

The **filename is the page name**, shown under the tile and read aloud when she
picks it, so name it in Hebrew.

- `_` and `-` become spaces: `בית-הקסמים.png` → "בית הקסמים". Real spaces are
  left alone.
...
- When two files would show the *same* text on their tiles, the tile falls back
  ...
  matches. She still hears the plain name read aloud.
...
The pipeline is `../src/world/draw/pageInk.js`: crop the desk away, divide out
the lighting so cream paper flattens to white, then key paper to transparent and
ink to black. The result floats above her canvas — she paints underneath and the
lines stay crisp.

Shoot for it:

- **Fill the frame with the paper.** The crop always keeps at least the middle
  ~30% of each axis (`TRIM_MAX` = 0.35 a side), so a desk margin wider than about
  a third of the frame leaves a strip inside the crop. Desk *beside* the page is
  wiped row by row, but a band left entirely **above or below** the page keys
  solid black across the top or bottom of her sheet.
- **Don't shoot it from across the room.** Under 8% of the frame being paper
  ...
  The desk is keyed away rather than turning black — but the drawing then sits
...
- **Square-on.** Perspective is never corrected; a keystoned page stays
  keystoned, and the desk triangles in the corners are cleaned up per row.
- **Full-bleed art** (ink running off the edge of the paper) is recognised —
  ...
  cap so it eats desk instead of artwork. Expect up to that 15% to go anyway, so
  don't count on a thin border surviving.
...
just makes her wait. Six full pages and 48 thumbnails stay cached after that.

## The button on the page — דף שקוף / קווים כהים

A family page puts one extra button in the top bar (built-in pages don't have
it). It is labelled by what the **tap will do**, not by what she is looking at:

- **דף שקוף** (ghost icon) — tap it and the lines fade to 22%, for tracing: she
  draws over the faint guide and ends up with her own lines.
- **קווים כהים** (pencil icon, highlighted) — showing while the page is faded.
  Tap to bring the dark lines back.

That is the whole explanation she needs: *"רואה את הכפתור? לחיצה אחת והקווים
נהיים חיוורים כדי שתוכלי לצייר עליהם, לחיצה שנייה והם חוזרים."*
...
| Page is always washed out, even after tapping קווים כהים | Keying failed and the app is showing the untouched photo faintly so she still has something to trace. Reshoot it. |
| Black band along the top or bottom | Too much desk in the frame — see the crop cap above. |
...
git commit -m "Add coloring page: חד-קרן"
git push
```
...
on the tablet. The live URL is the one in the repo's **Settings → Pages**, also
printed in the deploy step of the Actions run.

## Privacy — read this before you name a file

**This repo is public and the site sits at a guessable GitHub Pages URL.**
...
So name the *picture*, not the people, the place or the date: `חד-קרן.jpg`,
`טירה.png`, `פרפר.svg`. That is what she sees under the tile anyway, and a
neutral name reads just as well.
